...
def tax_predict(output_f):
	...
	d = pd.read_csv('clf_info_order.tsv',sep = '\t')
	...
	fd = pd.read_csv('clf_info_fam.tsv',sep = '\t')
	...
	fp_ord = os.path.join(output_f, 'clf_info_order.tsv')
	fp_fam = os.path.join(output_f, 'clf_info_fam.tsv')
	os.rename('clf_info_order.tsv', fp_ord)
	os.rename('clf_info_fam.tsv', fp_fam)
